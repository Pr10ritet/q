...
sub o_menu
{
 return	&ahref($scrpt,'Список отделов').
	($pr_edt_main_tunes && &ahref("$scrpt&op=new",'Новый отдел'));
}
...
sub o_show
{
 $OUT.=&form(%{$d->{form_header}},
   &Table('tbg3',
     ...
     &RRow('*','ll','Название отдела',&input_t('of_name',$of_name,26,127)).
...
sub o_save
{
 ...
 ($Fof_name eq '') && &Error("Дайте название отделу.$go_back",$tend);
