...
sub o_menu
{
 ...
 %c_grps=();
 $sth=&sql($dbh,"SELECT * FROM c_grps ".(!$PR{104} && "WHERE office=$Admin_office ")."ORDER BY name_grp");
 ...
 return &bold_br('Контакты').
   &form('#'=>1,'act'=>$Fact,"$h<br><br>".&input_t('text',$F{text},20,80)." <input type=submit value='Найти'>").$br2.
...
sub o_list
{
 ...
 $where.=" AND c.id_admin=$Admin_id" unless $Fgrp;
 $where.=" AND (c.id_admin=$Admin_id OR c.grp<>0)" if $Fgrp<0;
 $where.=" AND (g.office IN (0,$Admin_office) OR c.grp=0)" if $Fgrp && !$PR{104}; # нет прав просматривать контакты чужих отделов
 ...
 while ($p=$sth->fetchrow_hashref)
   {
    ...
    $h=!$grp || $PR{105} || ($PR{99} && ($office==$Admin_office || $office==0));
    $out.=&RRow('*','llllcc',
...
sub o_show
{
 $grps='контакт будет доступен только вам'.&input_h('grp'=>0) unless $PR{99};
 $OUT.=&form(%{$d->{form_header}},
   &Table('tbg3',
     &RRow('head','C',&bold_br($d->{name_action})).
     &RRow('*','ll','Группа контакта',$grps).
     &RRow('*','ll','Имя',&input_t('name_contact',$name_contact,50,127)).
...
sub o_getdata
{
 ...
 &Error("У вас нет доступа к чужим личным контактам.",$tend) if !$grp && $id_admin!=$Admin_id;
 &Error("У вас нет доступа к контактам других отделов.",$tend) if !$PR{104} && $grp && $office!=$Admin_office;
 $d->{priv_edit}=!$grp? 1 : $office!=$Admin_office? $PR{105} : $PR{99}; # если отдел другой, то установим требование права редактирования контактов чужих отделов
...
sub o_save
{
 ...
 $Fgrp=int $F{grp};
 $Fgrp=0 if $Fgrp<0;
 $Fgrp=0 if $Fgrp && !$PR{99}; # не разрешаем пероводить/создавать контакт в группе, если разрешено редактировать тока личные
 # Проверять группу не будем, пусть пуляет контакт куда хочет, поскольку мог бы его просто удалить
 $d->{sql}="id_admin=$Admin_id,name_contact='$Fname_contact',contact='$Fcontact',grp=$Fgrp";
