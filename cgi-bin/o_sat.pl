...
sub o_menu
{
 return	&ahref($scrpt,'Список сателлитов').
	($pr_edt_main_tunes && &ahref("$scrpt&op=new",'Новый сателлит'));
}

sub sat_load_config
{
 $satcfg_fname="$Nodeny_dir_web/satellite.cfg";
 open(FL,"<$satcfg_fname") or &Error("Ошибка загрузки файла-шаблона конфигурации сателлита <b>$satcfg_fname</b>!",$tend);
 @cfg_lines=<FL>;
 close(FL);
 my $ver=0;
 foreach (reverse @cfg_lines) {/^#(\d\d.\d\d)/ && ($ver=$1) && last}
 $ver==$VER_chk or &Error("Неверная версия файла $satcfg_fname: $ver");
}
 ...
 while ($p=$sth->fetchrow_hashref)
   {
    ...
    $out.=&RRow('*','llcc',
      ...
      $pr_edt_main_tunes && &ahref("$scrpt&op=del&id=$id",'Х')
    );
   }
 ...
 foreach $i (@cfg_lines)
   {
    ...
    $out.=&RRow('*','lll',&input_t("s_$i1",$value,40,255),$i3,&Filtr_out($default_value));
   }    
 $OUT.=&form(%{$d->{form_header}},
   &Table('tbg1i',
     &RRow('head','3',&bold_br($d->{name_action})).
     &RRow('*','lll','Логин сателлита',&input_t('login',$login,40,128),'С этим логином будет осуществляться коннект сателлита к базам данных').
     &RRow('*','lll','Имя сателлита',&input_t('name',$name,40,128),'').
     &RRow('*','lll','Комментарий',&input_ta('comment',$comment,44,3),'').$out.
     ($pr_edt_main_tunes && &RRow('head','3',$br.&submit_a('Сохранить').$br2))
   )
 );
}
...
sub o_save
{
 ...
 $d->{sql}="login='$Flogin',name='$Fname',comment='$Fcomment',config='$config',Passwd_Key='$Passwd_Key',version=$VER,time=$ut";
