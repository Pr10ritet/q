...
$d={
	...
	'tbl'		=> 'points',
	'field_id'	=> 'id',
	'priv_show'	=> $pr_topology,
	'priv_edit'	=> $pr_edt_topology,
};
 ...
 while ($p=$sth->fetchrow_hashref)
   {
    ...
    $out.=&RRow('*','clcclcccc',
      ...
      ($pr_edt_topology && &ahref("$scrpt&op=del&id=$id",'X'))
    );
   }
...
sub o_show
{
 ...
 $OUT.=&form(%{$d->{form_header}},
   &Table('tbg3',
     ...
     &RRow('*','lll','Номер точки',&input_t('box',$box||'',10,10),$show_map).
     &RRow('*','lll','Улица',$streets,'').
     &RRow('*','lll','Дом',&input_t('house',$house,10,10),'').
     &RRow('*','lll','Блок',&input_t('block',$block,10,10),'можно не число').
     &RRow('*','lll','Подъезд',&input_t('pod',$pod,10,10),'можно не число').
     &RRow('*','lll','Код подъезда',&input_t('cod_pod',$cod_pod,10,16),'').
     &RRow('*','lll','Неучтенных портов',&input_t('unknown_ports',$unknown_ports,10,10),'кол-во портов, к которым подключены неучтенные потребители').
     &RRow('*','lll','Соединения с',&input_t('connected',$connected,20,127),'номера точек через запятую, с которыми данная точка имеет соединение. Каждая точка уменьшает кол-во свободных портов в данной точке').
     &RRow('*','lll','Грозозащиты',&input_t('netprotects',$netprotects,20,127),'номера точек через запятую, с которыми данная точка имеет соединение через грозозащиту').
     &RRow('*','lll','Ключ от точки',&input_t('kl4',$kl4,20,127),'ключ от ящика и др. параметры точки').
     &RRow('*','lll','Питание',&input_t('power',$power,20,127),'сокращенно, например: Л - лифтовая, О - освещение, Щ - щиток, П - перегородка последнего этажа').
     &RRow('*','lll','Подсеть',&input_t('net',$net,20,20),'Начальный ip-адрес подсети, при внесении нового клиента будет предложен свободный ip начиная с этого адреса (сделайте буфер для `неклиентских` ip)').
     &RRow('*','lL','Комментарий',&input_ta('comment',$comment,40,8)).
     &RRow('head','3',$pr_edt_topology? &submit_a('Сохранить') : $go_back.$br2)
   )
 );
}
