...
{
   $i=$p->{class};
   ${"Ncls$i"}=&Filtr_out($p->{comment});
}
${"Ncls$_"}||="Направление $_" foreach (1..8);
$Ncls0="Общий";

# ---  меню слева  ---
$out=&div('row3',$br.'&nbsp;Сейчас '.&bold($time_now).$br2).
     &ahref("$a&c=4&ed=1",'Детализация');
$out.=&ahref("$a&c=5",'Персональная детализация') if $pr_detail_traf;
$out.=&ahref("$a&class=-1",'Неучтенный трафик').
     &ahref("$a&graf=1&class=1",'График');
$out.=&ahref("$a&c=1",'Суточная статистика') if $PR{92};
$out.=&ahref("$a&c=6",'Статистика авторизаций') if $PR{106};
$out.=&ahref("$scrpt0&a=monitoring",'Мониторинг') if $PR{106};

$OUT.="<table cellpadding=0 cellspacing=4 class=width100><tr><td valign=top>".&div('nav2',&MessX($out,0,1))."</td><$tc valign=top>";
$tend="</td></tr></table>";
...
sub GetUserInfo2
{
 ...
 $s='&nbsp;&nbsp;';
 my ($userinfo,$grp,$mId)=&ShowUserInfo($id);
 $UGrp_allow{$grp} or &Error("Клиент в группе, доступ к которой вам запрещен.",$tend);
 $aliases='';
 $allId='';
 $alias_count=-1;
 $sth=&sql($dbh,"SELECT id,mid,ip,name,paket FROM users WHERE mid=$mId OR id=$mId ORDER by mid",'Все записи клиента, включая основную и алиасы');
 while ($p=$sth->fetchrow_hashref)
 {
    $h=$p->{id};
    $aliases.=&RRow('*','lll',$s.$p->{ip}.$s,$s.&Filtr_out($p->{name}).$s,$s.&ahref("$a&mid=$h",'стат').$s);
    $alias_count++;
    $allId.="$h,";
    $paket=$p->{paket} unless $p->{mid};
 }
...
sub chanal_load
{
 !$Fclass && !$pr_detail_traf && &Error("Нет прав на просмотр детализации трафика.",$tend);
 ...
 $p or &Error($pr_SuperAdmin? 'Таблица traf_info, содержащая данные о ходе записи трафика, пуста. Проверьте запущено ли ядро NoDeny.':'Нет данных.',$tend);
 $tmax=$p->{time};
 ...
 if( $t>($trealmax+$i1) )
 {
    ...
    $i1=$i2 % 60;
    $i2=int $i2/60;
    $OUT.=&div('message lft',"Внимание. За последние $i2 часов $i1 минут нет информации о ходе записи трафика. Проверьте запущено ли ядро NoDeny.") if $pr_SuperAdmin;
 }
 ...
 $now_stat=$Fgraf? '' : ' зарегистрированная в '.&the_short_time($t_srez,$t);
 $now_stat.=!$Fclass? ' детализированная статистика': $Fclass<9? ' статистика трафика <span class=data1>'.${"Ncls$Fclass"}.'</span>' : ' статистика трафика всех направлений';
 ...
 {
    $WHERE='WHERE 1';
    $right_col.=&div('borderblue row1 lft',"Показана $now_stat всей сети").$br;
    $for_client='всей сети';
 }

 $a2="$a&start=$start";
 $right_col.=&ahref("$a&class=0",'Детализация текущего среза').$br if $Fclass && !$Fgraf && $pr_detail_traf;
 ...
 $right_col.='<div class=nav3>';
 $right_col.=&ahref("$a&class=9","По всем направлениям трафика $for_client").$br; 
 $right_col.=&ahref("$a&class=$_",${"Ncls$_"}.' '.$for_client) foreach (1..8);
 $right_col.='</div>';
  ...
  else
 {
    ...
    {
       $user_cell=!$pr_show_fio? 'IP' : $Fshowip? &CenterA("$a&start=$start&showip=0",'&rarr; Имя') : &CenterA("$a&start=$start&showip=1",'&rarr; IP');
       @from_to=("К клиенту, $ed","От клиента, $ed");
    }

    $header.="<table class='tbg width100'>".
       &RRow('head nav','6','Срез: '.&bold(&the_short_time($t_srez,$t)).'. Длительность среза: '.($t_period_text || 'не установлена')).
 ...
 ($sql,$page_buttons,$rows,$sth)=&Show_navigate_list("SELECT $select,time $sql_end",$start,$Max_line_chanal,$a,$dbs);

 $page_buttons&&=&RRow('tablebg',6,$page_buttons);
...
sub chanal_srez
{
 ...
 while ($p=$sth->fetchrow_hashref)
 {
  @cell=('','','','','','');
  {
   ...
   {
      # Не прогоняем через &sql_select_line - много инфы будет в $DOC->{admin_area}
      $t0_sql=[gettimeofday];
      $h=$dbh->prepare("SELECT * FROM users WHERE id=$mid LIMIT 1");
      $h->execute;
      $t0_sql=tv_interval($t0_sql);
      &DEBUG("(select user id=$mid - $t0_sql сек) ");
      $T_sql+=$t0_sql;
      if( $h=$h->fetchrow_hashref )
      {
         $ipp=$h->{ip};
         $name=$pr_show_fio? $h->{name} : $ipp;
         $grp=$h->{grp};
   ...
   $cell[1]=!$UGrp_allow{$grp}? '&nbsp;': $Fshowip? &ahref("$scrpt0&a=user&id=$mid'",$ipp,"title='$name'") : &ahref("$scrpt0&a=user&id=$mid'",$name,"title='$ipp'");
  }

  {
   if( $Fclass )
   {
      ...
      {  # выведем % загрузки канала
         $traf1*=100;
         $cell[4]=&tag('span',int($traf1/$max_traf_srez +.5),'class=data1');
      }
      last;
   }
   ...
   if( $preset>=0 )
   {
      ...
      $nclass=$nclass<0? &tag('span','ошибка','class=disabled') : ('Бесплатный',&Get_Name_Class($preset))[$nclass];
   ...
   if( $UGrp_allow{$grp} )
   {
      $cell[4]="$g $ipp $proto <span id=f$ip_id class='data2' style='cursor:pointer;border:0;' ".
        "onClick=window.open('$scrpt0&a=operations&act=resolv&ip_id=$ip_id&ip=$ipp','_blank','width=400,height=80')>Р</span>";
      $cell[5]=$nclass;
      $ip_id++;
   }else
   {
      $cell[4]=&tag('span','скрыто','class=disabled');
   }
  }
...
sub chanal_graf
{
 ...
 while( $p=$sth->fetchrow_hashref )
 {
    ($t_check,$traf1,$traf2)=&Get_fields('time','SUM(`in`)','SUM(`out`)');
 ...
 while( $t_check=shift @U )
 {
    ...
    $OUT.=&PRow."<td nowrap>&nbsp;";
    $OUT.=sprintf("%02d.%02d.%02d ",$tm->mday,$tm->mon+1,$tm->year-100) if $day_now!=$tm->mday || $mon_now!=($tm->mon+1);
    ...
    $OUT.=&tag('td',&Print_traf($traf1,$Fed,$t_period),'class=cntr');
    ...
    {  # выведем загрузку канала для текущей строки
       $OUT.=&tag('td',int($traf1*100/$max_traf +.5),"class='data1 cntr'");
    }else
    {
       $OUT.=&tag('td','&nbsp;');
    }
...
sub ShowTrafInfo
{
 ...
 $out="<table class='tbg1 width100'><tr class=head><$tc colspan=3>$out: ".&bold(&the_short_time($t1,$t))."</td></tr>";
...
sub list_detail
{
 &Error("Нет прав на просмотр детализации трафика.",$tend) if !$Fclass && !$pr_detail_traf;
 $sql="SELECT * FROM users WHERE detail_traf<>0 AND grp IN ($Allow_grp) ORDER BY sortip";
 $sth=&sql($dbh,"$sql LIMIT 1",'Список клиентов с персонально включенной детализацией');
 unless ($sth->rows)
 {
    $OUT.=&div('infomess','Нет клиентов с <em>персонально</em> включенной детализированной статистикой');
    return;
 }

 $out='';
 ($sql,$page_buttons,$rows,$sth)=&Show_navigate_list($sql,$start,$Max_list_users,"$scrpt&a=$Fa&c=5");
 while ($p=$sth->fetchrow_hashref)
 {
    $id=$p->{id};
    $out.=&PRow.($pr_show_fio? "<td>".(&Filtr_out($p->{name}) || '&nbsp;')."</td><td>".(&Filtr_out($p->{fio}) || '&nbsp;')."</td>" : '').'<td>'.
        &ahref("$a&a=user&id=$id",&Filtr_out($p->{ip}))."</td><$tc>".&ahref("$scrpt&a=$Fa&&mid=$id&graf=1&class=0",'Статистика').'</td></tr>';
 }

 $colspan=$pr_show_fio? 4 : 2;
 ...
 $OUT.=&Table('tbg1 width100',
     ...
     ($pr_show_fio? &RRow('head','cccc','Логин','ФИО','IP','Статистика') : &RRow('head','cc','IP','Статистика')).
     $page_buttons.$out
 );
}
...
sub day_stat
{
 ...
 $Fday=int $F{day};
 $Fmon=int $F{mon};
 $Fmon=$Fmon<1 || $Fmon>12? $mon_now : $Fmon;
 ...
 if( $Fmid )
 {
    ...
    $UGrp_allow{$p->{grp}}<2 && &Error("Клиент находится в недоступной вам группе. Просмотр суточной статистики заблокирован.",$tend);
    $ipp=$p->{ip};
 ...
 {
    $T_sql+=$t1_sql;
    &DEBUGX("Время переноса учетных записей в таблицу user_select: $t1_sql сек",0,0);
 }
 ...
 if( $Fmon==$mon_now && $Fyear==$year_now )
...
{
 &DB2_Connect;
 $t_srez=$Fwhen || $t;	# если срез не запрошен, то считаем что заказан текущий
 $Fed=int $F{ed};	# единица измерения
 ...
 # Пока mysql не оптимально обсчитывает SQL_CALC_FOUND_ROWS если в запросе есть LIMIT. Когда будет оптимально - тогда убрать $rows в &Show_navigate_list()
 $p=&sql_select_line($dbs,"SELECT COUNT(DISTINCT time) FROM $tname $where",'Вычислим общее количество строк, потому что пока mysql неоптимально считает SQL_CALC_FOUND_ROWS');
 $rows=$p? $p->{'COUNT(DISTINCT time)'} : 0;

 ($sql,$page_buttons,$rows,$sth)=&Show_navigate_list($sql,$start,$Max_line_chanal,$a,$dbs,$rows);

 $OUT.="<table class='tbg width100'>";
 $OUT.=&RRow('head nav','7c','Общая статистика трафика',&CenterA("$scrpt0&a=operations&act=help&theme=detail_help",'Справка')) unless $Fmid;
...
sub auth_stat
{
 $PR{106} or &Error("Нет прав на мониторинг системы.",$tend);
 ...
 foreach $auth_method ('Авторизатор','На порту','Web-авторизация','PPPoE')
 {
    ...
    while ($p=$sth->fetchrow_hashref)
    {
      ...
      if( $Fdetail )
      {
         $out.=&RRow('*','ll',&ahref("$scrpt0&a=user&id=$id",$U{$id}),$n);
      }
    }

    $out=&RRow('head','C','Метод авторизации: '.&bold($auth_method));
    foreach $g (sort {$UGrp_name{$b} cmp $UGrp_name{$a}} keys %UGrp_name)
    {
       $out.=&RRow('*','ll',$UGrp_name{$g},$sum{$g});
    }
