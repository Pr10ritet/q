...
$pr_show_fio or &Error('Доступ запрещен. Недостаточно привилегий.');
...
unless ($F{act})
  {
   ...
   $DOC->{header}.=qq{<meta http-equiv="refresh" content="0; url='$scrpt&act=check_now'">};
   &Exit;
  }

# md5.js актуальный? В md5.js функция SelfTest() делает div с id=selftestX невидимым
$OUT.='<div id=selftest1>'.&error('Скрипт md5.js не соответствует текущей версии!','Необходимо обновить. Обратитесь к главному администратору.').'</div>';
$DOC->{body_tag}.=qq{ onload="javascript: SelfTest();"}; 
...
if ($pr_SuperAdmin)
  {# суперадмин. Проверим, что есть доступ ко всем группам
   $out.=!$UGrp_allow{$_} && &commas($UGrp_name{$_}).',' foreach (keys %UGrp_name);
...
$where=!$pr_SuperAdmin && "u.grp IN ($Allow_grp) AND"; # для суперадмина не делать фильтр по группе т.к нужно вычислить клиентов в несуществующих группах
...
$sth=&sql($dbh,"SELECT * FROM users ".(!$pr_SuperAdmin && "WHERE grp IN ($Allow_grp)")." ORDER BY mid,sortip");
while ($p=$sth->fetchrow_hashref)
  {
   ...
   $out3.="<li>$client доступ не заблокирован, а пакет ".&commas($Plan_name_short[$paket])." указывает заблокировать.</li>" if $Plan_flags[$paket]=~/k/ && ($state ne 'off');
   ...
   if (!$mid)
   {  # основная запись
      $out1.="<li>$client в несуществующей группе № $grp</li>" if !$UGrp_name{$grp} && $pr_SuperAdmin;
   }
    else
   {  # алиасная запись
      if ($grp!=$grps{$mid})
