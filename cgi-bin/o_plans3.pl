...
sub o_menu
{
 return	&bold_br('Дополнительные тарифы').
        &ahref($scrpt,'Список').
	($pr_edt_tarifs && &ahref("$scrpt&op=new",'Создать новый')).
 ...
 while ($p=$sth->fetchrow_hashref)
   {
    ...
    $out.=&RRow('*','cllccc',
       ...
       (!$users && $pr_edt_tarifs && &ahref("$scrpt&op=del&id=$id",'X'))
    );
   }
...
sub o_show
{
 ...
 foreach $i (sort {$UGrp_name{$a} cmp $UGrp_name{$b}} grep $UGrp_allow{$_}, keys %UGrp_name)
   {
    $h=$UGrp_name{$i}.$br;
 ...
 $OUT.=&form(%{$d->{form_header}},
   &Table('tbg3',
     ...
     &RRow('*','ll','Название тарифа',&input_t('name',$name,40,127)).
     &RRow('*','ll',"Цена, $gr",&input_t('price',$price,15,30)).
     &RRow('*','ll',"Цена смены тарифа, $gr",&input_t('price_change',$price_change,15,30)).
     ...
     &RRow('*','ll','Описание тарифа (видно клиентам)',&input_ta('descr',$descr,30,5)).
     ($pr_edt_tarifs && &RRow('head','C',&submit_a('Сохранить')))
   )
 );
}
...
sub o_save
{
 ...
 $Fusr_grp=join ',', sort {$a <=> $b} grep $F{"grp_$_"}, keys %UGrp_name;
 $Fusr_grp_ask=join ',', sort {$a <=> $b} grep $F{"grpa_$_"}, keys %UGrp_name;
 $Fusr_grp=~s|(.+)|,$1,|;
 $Fusr_grp_ask=~s|(.+)|,$1,|;
