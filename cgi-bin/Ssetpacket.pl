...
{
 {
  ...
  &Insert_Event_In_DB(428,$Fpaket? $Plan_name_short[$Fpaket] : '');
...
{ 
 ...
 {# в порядке возрастания стоимости пакета
    ...
    for $j (1..4)
    {
       ...
       $mb_in_paket=${"Plan_mb$j"}[$i];
       $in_or_out_traf=&Get_name_traf(${"InOrOut$j"}[$i]);
       $out.=&RRow('*','lr',
          "$c[$j]) трафик. Предоплачено Мб<br>Оплачивается $in_or_out_traf",
          $mb_in_paket<$unlim_mb? $mb_in_paket : &bold('безлимитный')
       );
       $out.=&RRow('*','lr',"Цена превышения, $gr/Мб",$price_over_mb) if $mb_in_paket<$unlim_mb;
    }

    if (($Plan_mb1[$i]+$Plan_mb2[$i]+$Plan_mb3[$i]+$Plan_mb4[$i]+$Plan_over1[$i]+$Plan_over2[$i]+$Plan_over3[$i]+$Plan_over4[$i])==0)
    {
