...
sub o_menu
{
 ...
 return	&ahref("$scrpt&op=new&preset=$Fpreset",'Новое направление').
	...
	$out.$br.
	&ahref("$scrpt0&a=operations&act=help&theme=nets_help",'Справка');
}
...
sub o_list
{
 ...
 while ($p=$sth->fetchrow_hashref)
 {
     ...
     else 
    {
       ...
       $out.=&RRow('*','cr3ll',"Направление <b>$class</b>","<span class=data1>$comment</span> трафик",($port? 'таблица ipfw: '.&bold($port):''),$button_edit,$button_del);
    } 
 }
...
sub o_show
{
 ...
 $OUT.=&form(%{$d->{form_header}},
   &Table('tbg3',
     ...
     &RRow('*','ll','Номер направления',&input_t('class',$class,5,30)).
     ($priority?
       &RRow('*','ll','Приоритет',&input_t('priority',$priority,5,30)).
       &RRow('*','ll','Сеть',&input_t('net',$net,40,255)).
       &RRow('*','ll','Порт',&input_t('port',$port,5,10)).
       &RRow('*','ll','Комментарий',&input_t('comment',$comment,40,255)) :

       &RRow('*','ll','Название направления',&input_t('comment',$comment,40,128).' трафик').
       &RRow('*','ll','Номер таблицы фаервола',&input_t('port',$port,4,30)).
...
sub o_save
{
 ...
 $Fpriority && $Fnet!~/^(file:.+|\d+\.\d+\.\d+\.\d+(\/\d+)?)$/ && &Error('Описание направления не сохранено т.к '.
   'сеть задана неверно! Сеть должна быть задана в виде <b>xx.xx.xx.xx/yy</b>, <b>xx.xx.xx.xx</b> либо <b>file:имя файла</b>',$go_back.$tend);
