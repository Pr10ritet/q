...
sub o_menu
{
 return	&bold_br('Группы контактов').
	...
	($PR{99} && &ahref("$scrpt&op=new",'Новая группа')).$br.
...
sub o_list
{
 ...
 while ($p=$sth->fetchrow_hashref)
   {
    ...
    $h=$PR{99} && ($PR{105} || $office==$Admin_office); # 105 - ред. контакты чужих отделов
    $out.=&RRow('*','llccc',
      $name_grp,
      $Offices{$office} || '<span class=disabled>доступно всем отделам</span>',
      $n,
 ...
 $out or &Error(($PR{104}? 'Не создано ни одной группы контактов.' :
                  'Не создано ни одной группы контактов вашем отделе.').$br2.
    ($PR{99} && &ahref("$scrpt&op=new",'Создать группу')),$tend);
...
sub o_show
{
 if ($PR{105})
   ...
   {# в любом случае в скрытом поле сохраним номер отдела т.к в момент записи права на изменения в чужих отделах могут быть даны
    $offices=&input_h('office',$office).($Offices{$office} || 'ВСЕМ ОТДЕЛАМ');
   } 

 $OUT.=&form(%{$d->{form_header}},
   &Table('tbg3',
     &RRow('head','C',&bold_br($d->{name_action})).
     &RRow('*','ll','Название группы',&input_t('name_grp',$name_grp,50,127)).
     &RRow('*','ll','Будет доступна только отделу',$offices).
     &RRow('head','C',$PR{99} && ($PR{105} || $office==$Admin_office)? &submit_a('Сохранить') : "$go_back<br><br>")
   )
 );
}
...
sub o_getdata
{
 ...
 &Error("У вас нет доступа к контактам других отделов.",$tend) if !$PR{104} && $office && $office!=$Admin_office;
 $d->{priv_edit}=$office!=$Admin_office? $PR{105} : $PR{99}; # если отдел другой, то установим требование права редактирования контактов чужих отделов
 ...
 $_.=' отдела '.&commas($Offices{$office}) if $Offices{$office};
 $d->{name}=$_;
}
...
sub o_save
{
 ...
 !$PR{105} && $Foffice!=$Admin_office && &Error("Вам не разрешено редактировать контакты других отделов.",$tend);
