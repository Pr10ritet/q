...
sub o_menu
{
 return	&bold_br('Предустановленные подключения').
	...
	($pr_edt_main_tunes && &ahref("$scrpt&op=new",'Создать новое')).$br.
...
sub o_show
{
 ...
 $OUT.=&form(%{$d->{form_header}},&Table('tbg1',
    ...
    &RRow('*','lll','Название',&input_t('opt_name',$opt_name,50,127),'').
    &RRow('*','lll','Сумма снятия',&input_t('pay_sum',$pay_sum,50,127),'Сумма, на которую будет изменен баланс клиента и проведен соответствующий платеж сразу же после создания учетной записи. Число может быть как положительным (бонус) так и отрицательным. Нулевое значение отключает создание платежа.').
    ...
    &RRow('*','lll','Запланированное действие',&input_t('opt_action',$opt_action,50,127),'Если указать ненулевое значение, то сразу после создания учетной записи, в таблицу платежей будет записано специальное '.&commas('запланированное событие'),', которое автоматически будет выполнено через заданный промежуток времени. В данный момент автор предусмотрел только одно событие с кодом 1 - удваивание всех положительных платежей через сутки. Применяеся для акций. Жду пожеланий').
    &RRow('*','lll','Время запланированного действия',&input_t('opt_time',$opt_time,50,127),'Количество секунд после создания учетной записи, через которое будет выполнено запланированное событие.').
    &RRow('*','lll','Комментарий к платежу',&input_ta('pay_comment',$pay_comment,38,6),'Комментарий, который будет установлен в платеже снятии за подключение. Например, укажите '.&commas('Подключение по акции')).
    &RRow('*','lll','Дополнительные данные',&input_ta('pay_reason',$pay_reason,38,6),'Допустим, при создании подключения в обязательном порядке требуется зарегистрировать какие-либо данные, например, в акционном подключении '.&commas('подключись по флаеру').' необходимо указать номер флаера дабы избежать махинаций со стороны персонала. '.
    ...
    &RRow('*','lll','Комментарий',&input_ta('opt_comment',$opt_comment,38,6),'Комментарий к данному подключению. Будет выводиться исключительно администратору чтобы он в списке подключений мог выбрать то, которое нужно.').
    ($pr_edt_main_tunes && &RRow('head','3',&submit_a('Сохранить')))
 ));
}
